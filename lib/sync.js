...
function buildCommonUtilsCode(commonUtils) {
  const code = String(commonUtils?.code || "").trim();
  if (!commonUtils?.enabled || !code) return "";

  if (!COMMON_UTILS_EXPORT_RE.test(code)) {
    console.warn("[cus:user-script] common utils must start with export const utils =");
    return "";
  }

  return code.replace(COMMON_UTILS_EXPORT_RE, "const utils =");
}

function buildCombinedCode(script, commonUtils) {
  return [buildCommonUtilsCode(commonUtils), script.code]
